...
const smsRowner = () => { return `\`\`\`¡¡ESTE COMANDO SÓLO 〘Noⷦvͣaͥ-Liⷦoͤnͯ'sͭ〙 COMO CREADORES DEL BOT LO PUEDE USAR!!\`\`\`` }//NUMERO DE BOT
const smsOwner = () => { return `\`\`\`¡¡ESTE COMANDO SÓLO 〘Noⷦvͣaͥ-Liⷦoͤnͯ'sͭ〙 LO PUEDE USAR!!\`\`\`` }//OWNER
const smsMods = () => { return `\`\`\`¡¡ESTE COMANDO SÓLO MODERADORES Y 〘Noⷦvͣaͥ-Liⷦoͤnͯ'sͭ〙 LO PUEDEN USAR!!\`\`\`\`` }//MODERADORES
const smsPremium = () => { return `\`\`\`¡¡ESTE COMANDO SÓLO ESTA DISPONIBLE PARA USUARIOS PREMIUM Y 〘Noⷦvͣaͥ-Liⷦoͤnͯ'sͭ〙!! PARA SER PREMIUM COMPRE UN PASE USANDO #pass premium\`\`\`` }//USUARIOS PREMIUM
...
//Aquí va el comando verificar

const smsRestrict = () => { return `\`\`\`¡¡ESTE COMANDO ESTÁ RESTRINGIDO POR 〘Noⷦvͣaͥ-Liⷦoͤnͯ'sͭ〙!!\`\`\`` }//RESTRINGIR COMANDO
...
//_allantilink.js
const smsTextoYT = () => { return '👻🦁 𝗦𝘂𝗽𝗲𝗿 〘Noⷦvͣaͥ-Liⷦoͤnͯ'sͭ〙 - 𝗪𝗵𝗮𝘁𝘀𝗔𝗽𝗽 '} 
...
const eGataCoins = () => { return '👻🦁 NovaLionsCoins' }
const eGataTickers = () => { return '🎫 NovaLions Tickets' } 
...
//propietario(a).js
const smsJoin = (user) => { return `${packname}\n𝙀𝙎 𝙐𝙉 𝘽𝙊𝙏 𝘿𝙀 𝙒𝙃𝘼𝙏𝙎𝘼𝙋𝙋 𝙌𝙐𝙀 𝙏𝙀 𝘼𝙔𝙐𝘿𝘼𝙍𝘼 𝙍𝙀𝘼𝙇𝙄𝙕𝘼𝙍 𝘿𝙄𝙁𝙀𝙍𝙀𝙉𝙏𝙀𝙎 𝘼𝘾𝙏𝙄𝙑𝙄𝘿𝘼𝘿𝙀𝙎 🪄 𝘼𝙇 𝙋𝙍𝙄𝙑𝘼𝘿𝙊 𝙊 𝙂𝙍𝙐𝙋𝙊 𝙔 𝙏𝘼𝙈𝘽𝙄𝙀𝙉 𝙏𝙀 𝙑𝘼𝙎 𝘼 𝘿𝙄𝙑𝙀𝙍𝙏𝙄𝙍 🎈 𝘾𝙊𝙉 𝙎𝙐𝙎 𝙈𝙐𝙇𝙏𝙄𝙋𝙇𝙀𝙎 𝙁𝙐𝙉𝘾𝙄𝙊𝙉𝙀𝙎, 𝘿𝙄𝙎𝙁𝙍𝙐𝙏𝘼 𝘿𝙀 〘Noⷦvͣaͥ-Liⷦoͤnͯ'sͭ〙!!! 😸\n\n💖 〘Noⷦvͣaͥ-Liⷦoͤnͯ'sͭ〙 𝙁𝙐𝙀 𝙄𝙉𝙑𝙄𝙏𝘼𝘿𝘼 𝙋𝙊𝙍:\n@${user}`}
...
const smsSoloOwnerJB = () => { return `${lenguajeGB['smsAvisoAG']()}*ESTE COMANDO ESTÁ DESACTIVADO POR 〘Noⷦvͣaͥ-Liⷦoͤnͯ'sͭ〙*`}
...
const smsContAdult = () => { return `${lenguajeGB['smsAvisoAG']()}*LOS COMANDOS 🔞 ESTÁN DESACTIVADOS, SI USTED ES 〘Noⷦvͣaͥ-Liⷦoͤnͯ'sͭ〙 USE #on modohorny*`}
